...
common.chart = (function() {
    ...
    function redraw(datum) {
        ...
        y.domain([0, d3.max(datum, function(d) { return d.traffic; })]);
        x2.domain(x.domain());
        y2.domain(y.domain());
    ...
    return {
        init:function(id) {
            ...
            area2 = d3.area()
                ...
                .y0(height2)
                .y1(function(d) { return y2(d.traffic); });
            ...
            var sample_data = [];
            for(var i = 200; i >= 0; i--) {
                var sample_date  = moment(new Date()).add(-i,"day").format("YYYY-MM-DD");
                var sample_traffic = i % 18 === 3 ? Math.random()*100 : Math.random()*10;
                sample_data.push({date:sample_date, traffic:sample_traffic});
            }
            redraw(sample_data)
        },
        uninit:function() {
